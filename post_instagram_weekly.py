...
def post_instagram(img_url: str, caption: str) -> bool:
    ...
    if "error" in resp:
        err = resp["error"]
        print(f"  IG container API error ({err.get('code','?')}): {err.get('message','')}")
        if err.get("code") in (190, 102, 467):
            print("  Token is expired or invalid — refresh INSTAGRAM_PAGE_TOKEN in GitHub Secrets")
        return False
    cid = resp.get("id")
    if not cid:
        print(f"  IG container error: {resp}")
        return False
    ...
    for attempt in range(12):
        ...
        if "error" in poll:
            err = poll["error"]
            print(f"  IG poll API error ({err.get('code','?')}): {err.get('message','')}")
            return False
    ...
    if "error" in resp:
        err = resp["error"]
        print(f"  IG publish API error ({err.get('code','?')}): {err.get('message','')}")
        return False
    ok = "id" in resp
    if not ok:
        print(f"  IG publish error: {resp}")
    return ok
...
def post_facebook(img_url: str, caption: str) -> bool:
    ...
    if "error" in resp:
        err = resp["error"]
        print(f"  FB API error ({err.get('code','?')}): {err.get('message','')}")
        if err.get("code") in (190, 102, 467):
            print("  Token is expired or invalid — refresh FACEBOOK_PAGE_TOKEN in GitHub Secrets")
        return False
    ok = "id" in resp
    if not ok:
        print(f"  FB post error: {resp}")
    return ok
